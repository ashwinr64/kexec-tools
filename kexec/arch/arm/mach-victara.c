...
static uint32_t victara_dtb_compatible(void *dtb, struct msm_id *devid, struct msm_id *dtb_id)
{
    int root_offset;
    const void *prop;
    int len;
...
static int victara_choose_dtb(const char *dtb_img, off_t dtb_len, char **dtb_buf, off_t *dtb_length)
{
    ...
    while(dtb + sizeof(struct fdt_header) < dtb_end)
    {
        ...
        if(victara_dtb_compatible(dtb, &devid, &dtb_id))
        {
...
static int victara_add_extra_regs(void *dtb_buf)
{
    FILE *f;
    uint32_t reg;
    int res;
...
const struct arm_mach arm_mach_victara = {
    .boardnames = { "victara", "bacon", "d851", "d855", NULL },
    .choose_dtb = victara_choose_dtb,
    .add_extra_regs = victara_add_extra_regs,
};
